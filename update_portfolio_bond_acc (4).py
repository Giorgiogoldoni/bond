...
UNIVERSE = [
    ...
    {"ticker":"OM3J.MU",  "name":"PIMCO US HY Acc (gettex)",  "cat":"HY_USD",   "risk":1.1,"cur":"EUR"},
]
...
# ── OTTIMIZZAZIONE PESI ───────────────────────────────────────────
def optimize_weights(scores: dict, prev_weights: dict, scenarios: dict) -> tuple:
    """Ritorna (weights_dict, exclusion_reasons_dict)."""
    ...
    xeon = "XEON.MI"
    if not is_crisis and not is_tightening:
        exclusion[xeon] = "Regime non di crisi/tightening"
    ...
    for tk, sc in sorted_etf:
        cat = etf_map[tk]["cat"]
        # Controlla vincoli categoria PRIMA del limite posizioni
        # cosi il motivo e' specifico e non generico
        if cat in GOVT_CATS and govt_count >= 4:
            exclusion[tk] = "Max 4 Govt IG raggiunto"
            continue
        if cat in HY_CATS and hy_count >= 2:
            exclusion[tk] = "Max 2 HY raggiunto"
            continue
        if cat in EM_CATS and em_count >= 2:
            exclusion[tk] = "Max 2 EM raggiunto"
            continue
        if cat in AGG_CATS and agg_count >= 1:
            exclusion[tk] = "Max 1 Aggregate raggiunto"
            continue
        if len(selected) >= MAX_POSITIONS:
            exclusion[tk] = "Score insufficiente (max " + str(MAX_POSITIONS) + " posizioni)"
            continue
        if cat in GOVT_CATS:
            govt_count += 1
        elif cat in HY_CATS:
    ...
    final = {tk: w for tk, w in rounded.items() if w > 0}
    return final, exclusion
...
def run():
    ...
    print("\nOttimizzazione pesi...")
    weights, exclusion = optimize_weights(f_sc, prev_weights, scenarios)
    ...
    # ETF esclusi con score e motivo
    excluded_list = []
    for etf in sorted(UNIVERSE, key=lambda e: f_sc.get(e["ticker"], 0), reverse=True):
        tk = etf["ticker"]
        if tk in weights:
            continue
        pd_ = prices.get(tk, {})
        excluded_list.append({
            ...
            "ret_4w":           round(pd_.get("r4w") or 0, 2),
            "ret_12w":          round(pd_.get("r12w") or 0, 2),
            "exclusion_reason": exclusion.get(tk, "Score insufficiente"),
        })
