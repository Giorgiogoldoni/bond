...
def fetch_history_bulk(tickers: list, start: str) -> dict:
    ...
    Se un ticker torna vuoto/delistato con il suo suffisso originale, riprova
    individualmente con suffissi di borsa alternativi (.L Londra, .PA Parigi,
    .DE Xetra, .MI Milano, .AS Amsterdam) prima di arrendersi — stessa logica
    di fallback gia' presente in bond_momentum_strategies.fetch_prices_extended,
    qui mancava e causava buchi su ticker realmente disponibili ma quotati
    su una borsa diversa da quella attesa.
    """
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        for tk in batch:
            try:
                ...
                print(f"  ⚠ {tk}: dati non estraibili al primo giro ({e})")
        time.sleep(1.5)  # rispetto rate limit tra batch

    # ── RETRY con suffissi alternativi per ticker rimasti vuoti ──
    empty = [tk for tk in tickers if not out[tk]]
    if empty:
        print(f"\n{len(empty)} ticker con 0 punti dopo il primo giro, riprovo con suffissi alternativi: {empty}")
        alt_suffixes = [".MU", ".L", ".PA", ".DE", ".MI", ".AS", ".SW"]
        for tk in empty:
            ...
            for suf in alt_suffixes:
                ...
                if alt_tk == tk:
                    continue
                try:
                    hist = yf.Ticker(alt_tk).history(start=start, end=end, auto_adjust=True)
                    closes = hist["Close"].dropna()
                    if len(closes) > 20:
                        for dt, px in closes.items():
                            out[tk][dt.strftime("%Y-%m-%d")] = round(float(px), 4)
                        print(f"  ✓ {tk} trovato come {alt_tk}: {len(closes)} punti")
                        break
                except Exception:
                    continue
                time.sleep(0.5)
            if not out[tk]:
                print(f"  ✗ {tk}: NON TROVATO su nessun suffisso alternativo — verificare manualmente il ticker")
...
def run(start=BACKTEST_START, dry_run=False):
    ...
    # Export storico prezzi grezzo — serve alla pagina Renko satellite,
    # che ha bisogno delle serie daily-close reali, non solo dei pesi settimanali.
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    price_history_path = Path(__file__).parent / "data" / "price_history_all.json"
    price_history_path.write_text(
        json.dumps(history_bulk, ensure_ascii=False), encoding="utf-8"
    )
    print(f"\n→ scritto {price_history_path} (storico prezzi grezzo per Renko, {len(history_bulk)} ticker)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start", default=BACKTEST_START, help="Data inizio backtest YYYY-MM-DD")
